"""
Special handling of importing RGBMatrixEmulator for emulator
and real use with hardware.

Whenever RGBMatrixEmulator modules are needed, use:
...
if os.getenv("USE_RGB_MATRIX_EMULATOR", False):
    ...
    from rgbmatrix import graphics, RGBMatrix, RGBMatrixOptions, Canvas, Color
...
COLOR_7C_GOLD = graphics.Color(
    241, 194, 50
)  # f1c232 - a yellowish color, that is in harmony with this blue #6fa8dc and green #93c47d colors
...
FONTS_V0 = [FONT_XL_SDK, FONT_L_SDK, FONT_M_SDK, FONT_S_SDK, FONT_XS_SDK, FONT_XXS_SDK]
...
def draw_text(
    canvas, x: int, y: int, text: str, font=FONT_DEFAULT, color=COLOR_DEFAULT
):
    return graphics.DrawText(canvas, font, x, y, color, text)


def draw_grid(canvas, w: int, h: int, rows=4, cols=4, color=COLOR_GREY_DARKEST):
    x_step_size = w // cols
    for i in range(cols):
        x = i * x_step_size
        graphics.DrawLine(canvas, x, 0, x, h, color)
    y_step_size = h // rows
    for i in range(rows):
        y = i * y_step_size
        graphics.DrawLine(canvas, 0, y, w, y, color)
...
def _debug_font_info(font, name=""):
    print(f"Font {name} h={font.height} bl={font.baseline} y_off={y_font_offset(font)}")
...
def truncate_text(
    text: str, max_length: int, num_rows: int = 2, ellipsize: bool = False
) -> List[str]:
    words = text.split()
    result = []
    row = ""
