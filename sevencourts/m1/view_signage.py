...
def _display_team_player(
    canvas, x0: int, y0: int, player_index: int, name: str, flag_code: str
):
    font = FONT_SIGNAGE_TEAM_NAME

    # relevant only for doubles, otherwise 0
    x_shift = player_index * (
        W_FLAG_SMALL + 1 + width_in_pixels(font, " " * MAX_LENGTH_NAME_DOUBLES)
    )
...
def _display_team(
    canvas, x0: int, y_team: int, team, score_sets, is_serving: bool, score_game: str
):
    ...
    if team["p1"]["name"] is not None:
        ...
        return _display_team_score(
            canvas, x0, y_team, score_sets, score_game, is_serving
        )
    else:
        return 0
...
def _display_match(
    ...
    x_court_name = x0
    y_court_name = y0
    y_shift = _display_court_name(
        canvas, x_court_name, y_court_name, court_name, match_status
    )
    ...
    y_shift = _display_team(
        canvas, x0, y_t1, team1, score_sets_t1, is_serving_t1, score_game_t1
    )
